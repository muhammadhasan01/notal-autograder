...
HEADER = ['exam_name', 'number', 'nim', 'grade', 'total_node_cost', 'total_edge_cost', 'node_count', 'edge_count']
RESULT_FILE_NAME = 'Train'
...
class Grader:
    ...
    def __grade_one_on_one(self, graph_source: Graph,
        ...
        return score * Constants.MAX_SCORE, new_ub_normal_cost, total_node_cost, total_edge_cost
    ...
    def grade(self, graph_source: Graph,
        ...
        for graph_target in graph_targets:
            ...
            if cur_time - start_time < time_limit * 1000000:
                ...
                try:
                    score, cur_ub_normal_cost, cur_node_cost, cur_edge_cost = self.__grade_one_on_one(graph_source,
                                                                                                      graph_target,
                    ...
                    if mx_score < score:
                        mx_score = score
                        total_node_cost = cur_node_cost
                        total_edge_cost = cur_edge_cost
                        node_count = len(graph_source.nodes) + len(graph_target.nodes)
        ...
        return score, total_node_cost, total_edge_cost, node_count, edge_count
    ...
    for exam, dir_exam in EXAM_DIRECTORIES.items():
        ...
        for nim, graph in graph_inputs.items():
            if nim == ANSWER_NAME_FILE:
                continue
            grade, total_node_cost, total_edge_cost, node_count, edge_count = grader.grade(graph, [graph_answer], 3000,
                                                                                           3000,
                                                                                           ...
                                                                                           edge_cost=edge_cost,
                                                                                           is_exact_computation=is_exact_computation)
            results.append((exam_name, number, nim, grade, total_node_cost, total_edge_cost, node_count, edge_count))
...
def main():
    ...
    for relabel_method in RelabelMethod.__iter__():
        for graph_preprocess_type in GraphPreprocessType.__iter__():
            ...
            filename = f'{RESULT_FILE_NAME}_{relabel_method.name}_{graph_preprocess_type.name}.csv'
